...
const owner = process.env.GITHUB_REPOSITORY_OWNER || process.env.SITE_OWNER || 'drmsidhi';
...
for (const route of routes) {
  ...
  const base = route ? '../' : './';
  const page = template.replaceAll('https://drmsidhi.github.io/kanhaiya-convertor/', siteUrl).replace('<head>', `<head><base href="${base}">`).replace(/<link rel="canonical" href="[^"]+"\/>/, `<link rel="canonical" href="${url}"/>`).replace(/<meta property="og:url" content="[^"]+"\/>/, `<meta property="og:url" content="${url}"/>`).replace(/<title>[^<]+<\/title>/, `<title>${title}</title>`);
  await writeFile(`${path}/index.html`, page);
}
const configPath = 'dist/config/site.js'; await writeFile(configPath, (await readFile(configPath, 'utf8')).replace('https://drmsidhi.github.io/kanhaiya-convertor/', siteUrl));
